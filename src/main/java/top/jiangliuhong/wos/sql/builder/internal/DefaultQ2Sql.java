...
import top.jiangliuhong.wos.sql.builder.*;
import top.jiangliuhong.wos.sql.exception.ParsingException;
import top.jiangliuhong.wos.sql.exception.QSyntaxException;
import top.jiangliuhong.wos.sql.exception.SqlBuildException;
...
import top.jiangliuhong.wos.sql.support.XSingleSourceSupport;
...
public final class DefaultQ2Sql implements Q2Sql, ResultKeyGenerator, SourceScriptOptimizable, XSingleSourceSupport {
    ...
    public String toSql(Parsed parsed, Qr qr, DialectSupport dialectSupport) {
        ...
        if (SqlStringUtil.isNotNull(dialectSupport.getLimitOne()) && qr.getLimit() > 0) {
        ...
        for (Bb bb : refreshList) {
            ...
            if (bb.getP() == Op.X) {
                ...
                if (key.contains("?")) {
                    ...
                    if (be == null) {
                        throw new ParsingException("can not find the property " + key + " of " + parsed.getTableName());
                    }

                    TimeSupport.testWriteNumberValueToTime(be.getClz(), bb);

                    if (SqlStringUtil.isNullOrEmpty(String.valueOf(bb.getValue()))
    ...
    private void tryToFixBbValue(Bb bb,BeanElement be, DialectSupport dialectSupport) {
        ...
        if (be.isJson()) {
            ...
            if (v != null) {
                String str = SqlJsonUtil.toJson(v);
    ...
    private void parseAlia(Q q, SqlSth sqlSth) {
        ...
        if (q instanceof Q.X xq) {
            ...
            for (Froms sc : xq.getSourceScripts()) {
                if (SqlStringUtil.isNotNull(sc.getSource())) {
