...
import top.jiangliuhong.wos.sql.util.ParserUtil;
...
public interface Mapper {
    ...
    default String mapping(String key, Mappable mappable) {
        ...
        if (key.contains(Script.DOT)) {
            ...
            String clzName = ParserUtil.getTableName(alia, mappable.getAliaMap());
            ...
            if (SqlStringUtil.isNullOrEmpty(m)) {
                String s = mappable.getResultKeyAliaMap().get(key);
                if (SqlStringUtil.isNullOrEmpty(s))
    ...
    default Class mapClzz(String key, Mappable mappable) {
        ...
        if (key.contains(Script.DOT)) {
            ...
            Parsed parsed = Parser.get(clzName);
            if (parsed != null) {
                return parsed.getField(property).getType();
            } else {
                return String.class;
            }
        }
        ...
        if (parsed != null) {
            Field lastTest = parsed.getField(key);
            if (lastTest == null) {
                throw new SqlBuildException("not found property: " + key);
            }
            return lastTest.getType();
        }
